...
data = pickle.load(open(_data('regionC_data.pkl'),'rb'))
total = sp.sympify(data['total'], locals={'theta':theta,'t':t})
vol_fixed = sp.Rational(11,2)
...
for tt in [0.02, 0.08, 0.169918, 0.3, 0.55, 0.615480, 0.7, 0.78]:
    th_lo = theta_lo_of_t(tt)
    th_hi = theta_A(tt)
    thm = 0.5*(th_lo+th_hi)
    uu = np.tan(thm/2); vv=np.tan(tt/2)
    print(f"t={tt:.4f} theta=[{np.degrees(th_lo):.2f},{np.degrees(th_hi):.2f}] "
          f"u={uu:.4f} v={vv:.4f}  N={f(uu,vv):.6f} D={g(uu,vv):.6f} N/D={f(uu,vv)/g(uu,vv):.6f}")

with open(_os.path.join(_DATA_DIR, 'regionC_ND.pkl'), 'wb') as fp:
