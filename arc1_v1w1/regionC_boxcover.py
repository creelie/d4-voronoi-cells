...
data = pickle.load(open(_data('regionC_ND.pkl'),'rb'))
...
def theta_A_num(tt): return 2*np.arctan(np.cos(tt))
def theta_lo_of_t(tt):
    if tt < t1: return theta_max_Y(tt)
    elif tt < t3: return np.pi/3
    else: return 2*np.arctan(np.sin(tt))

MARGIN_V_END = 0.0007
v_lo_g = sp.nsimplify(round(0+MARGIN_V_END,6))
v_hi_true = np.tan((np.pi/4 - 0.0015)/2)
v_hi_g = sp.nsimplify(round(v_hi_true,6))
...
for k in range(NBOX):
    ...
    th_lo_box = max(theta_lo_of_t(tt_lo), theta_lo_of_t(tt_hi))
    th_hi_box = min(theta_A_num(tt_lo), theta_A_num(tt_hi))
