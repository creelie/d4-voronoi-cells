...
data = pickle.load(open(_data('regionE_data.pkl'),'rb'))
total = sp.sympify(data['total'], locals={'theta':theta,'t':t})
vol_fixed = sp.Rational(17,4)
...
def theta_hi_of_t(tt):
    if tt < t3:
        return 2*np.arctan(np.sin(tt))  # ceiling = W-curve
    else:
        return np.pi/3  # ceiling = U

for tt in [0.47, 0.5, 0.55, 0.6, 0.615480, 0.65, 0.7, 0.78]:
    th_lo = theta_max_Y(tt)
    th_hi = theta_hi_of_t(tt)
    thm = 0.5*(th_lo+th_hi)
...
with open(_os.path.join(_DATA_DIR, 'regionE_ND.pkl'), 'wb') as fp:
