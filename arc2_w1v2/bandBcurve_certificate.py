...
#!/usr/bin/env python3
"""
bandBcurve_certificate.py

The B-bounded sub-part of the second arc's band below region top,
bounded above by W' and below by B, for s in (s_3, pi/4).  This is the
one region in the paper where the direct symbolic route does not
complete, and this script is the record of exactly where it stops.

It reaches Step 2 without difficulty: the combinatorial pattern is
stable across the sub-interval (34 vertices, 11 moving, 23 fixed), and
the exact fixed-volume contribution is 6.  The moving-volume step is
...
The script therefore runs under an explicit time budget.  When the
budget is exhausted in the moving-volume loop it reports the recorded
outcome and stops, rather than running indefinitely.  Set
BANDB_BUDGET_SECONDS to change it (default 240), and
BANDB_PER_SIMPLEX_SECONDS to bound a single determinant (default 90); set it to 0 to let the
loop run without a budget.
"""
import sys, re, time, pickle, os, itertools

BUDGET_SECONDS = float(os.environ.get("BANDB_BUDGET_SECONDS", "240"))
PER_SIMPLEX_SECONDS = float(os.environ.get("BANDB_PER_SIMPLEX_SECONDS", "90"))
...
def _alarm(seconds):
    """Bound one symbolic call.

    Several of the determinants here do not settle in any practical time,
    so the loop budget alone is not enough: without a per-call bound a
    single cancel() can run for hours past it.
    """
    import contextlib
    import signal
...
T_LO, T_HI = 0.615480, np.pi/4

def check_stability():
    ts = np.linspace(T_LO+0.005, T_HI-0.005, 8)
    pats = []
    for tt in ts:
        th = 0.5 * (theta_B(tt) + theta_W(tt))
...
def derive_band():
    t0 = 0.5 * (T_LO + T_HI)
    theta0 = 0.5 * (theta_B(t0) + theta_W(t0))
    ...
    samples = [(tt, 0.5 * (theta_B(tt) + theta_W(tt))) for tt in np.linspace(T_LO+0.01, T_HI-0.01, 5)]
    ...
    for si, s in enumerate(moving_simplices):
        if BUDGET_SECONDS and time.time() - loop_start > BUDGET_SECONDS:
            print()
            print("=" * 70)
            print("MOVING-VOLUME STEP DID NOT COMPLETE")
            print("=" * 70)
            print("    %d of %d moving simplices reduced before the %.0fs budget"
                  % (si, len(moving_simplices), BUDGET_SECONDS))
            print("    expression growth in the remaining determinants is")
            ...
            print("    sub-interval, and the exact fixed-volume contribution 6.")
            raise SystemExit(0)
        ts0 = time.time()
        ...
        sign = 1 if np.linalg.det(Mnum) > 0 else -1
        try:
            with _alarm(PER_SIMPLEX_SECONDS):
                term = sp.cancel(sign * M.det() / 24, extension=True)
        except _Timeout:
            print("    simplex %d/%d: no closed form within %.0fs"
                  % (si, len(moving_simplices), PER_SIMPLEX_SECONDS), flush=True)
            print()
            print("=" * 70)
            print("MOVING-VOLUME STEP DID NOT COMPLETE")
            print("=" * 70)
...
def main():
    print("=" * 70)
    print("w1-v2 arc, band below region-top, B-sub-part (t in (t3,pi/4))")
    ...
    cache_path = _data("bandBcurve_step2_cache.pkl")
    ...
    cache_path3 = _data("bandBcurve_step3_cache.pkl")
    theta0_chk, t0_chk = 0.5 * (theta_B(0.5*(T_LO+T_HI)) + theta_W(0.5*(T_LO+T_HI))), 0.5*(T_LO+T_HI)
