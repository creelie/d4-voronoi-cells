...
an intermediate distance, between $2+\varepsilon_0$ and $\sqrt6$. A numerical search of that case
...
$\sum_i\delta_i\le4\cdot10^{-5}$, and their directions $W$ satisfy
...
(a) \emph{Rigidity with slack: $\|\varepsilon\|\le\frac{384}{71}(1+\delta)S$.} The
...
$\kappa\le2(1+\delta)S$. Consequently
$\sum_ie_i\le\sqrt{24}\,\|\varepsilon\|\le26.5\,S$ and
$\Theta\le\|\varepsilon\|+S\le6.42\,S\le2.6\cdot10^{-4}$.

(b) \emph{The volume to first order, with its error.} Move the data
...
$P(0)=Q$ and $P(1)=V(Y)$. Each facet of $P(t)$ moves with normal velocity
...
\]
$A_i$ the $3$-volume of the $i$-th facet $F_i(t)$ and $M_i=\int x\,dA$
over it. At $t=0$ the facet $F_i^0$ is a regular octahedron of volume
$\frac43$, centred at $u_i$ and symmetric about it, with circumradius $1$
in its hyperplane, and every other constraint has slack at least $\tfrac12$
at $u_i$, since $\langle u_i,u_j\rangle\le\frac12$. Two homotheties about
$u_i$ trap $F_i(t)$. For the inner one put
$\sigma=(\frac12\delta+\sqrt2e)/(1-e)$ and $\lambda_0=2(\sqrt2e+\sigma(1+e))$:
for $z\in F_i^0$ the point $u_i+(1-\lambda_0)(z-u_i)$ has
$\langle\cdot,u_j\rangle\le1-\frac12\lambda_0$ for $j\ne i$, and moving it
along $u_i$ by at most $\sigma$ into the hyperplane of $F_i(t)$ gives a point
with $\langle\cdot,w_j(t)\rangle\le1-\frac12\lambda_0+\sqrt2e+\sigma(1+e)\le h_j(t)$,
a point of $F_i(t)$; the move along $u_i$ does not decrease $3$-volume, so
$A_i(t)\ge\frac43(1-\lambda_0)^3$. For the outer one, $P(t)$ lies in the
ball of radius $r_1=\sqrt2(1+\frac12\delta)/(1-\sqrt2e)$, because
$\langle x,u_j\rangle\le h_j+|x|e$ and $\max_j\langle x,u_j\rangle\ge|x|/\sqrt2$;
so a point $x$ of $F_i(t)$ has $|\langle x,u_i\rangle-1|\le\eta_1=\frac12\delta+r_1e$,
and its projection $y$ along $u_i$ onto the hyperplane of $F_i^0$ has
$\langle y,u_j\rangle\le1+\eta_2$, $\eta_2=\eta_1+\frac12\delta+r_1e$, which puts
$y$ in $u_i+(1+2\eta_2)(F_i^0-u_i)$, since shrinking towards $u_i$ by the
factor $1+2\eta_2$ restores every constraint. The facet is the lift of
these projections, with area element multiplied by at most
$(1+e)/(1-e)$. As $F_i^0$ is symmetric about $u_i$, the moment
$\int(y-u_i)$ over the projection is at most the volume between the two
homothets times $1+2\eta_2$, and collecting the terms,
\[
  \frac{|M_i(t)-\tfrac43u_i|}{|w_i(t)|}\ \le\ \mu,\qquad
  \lambda_0\le6.7\,\Theta,\quad \mu\le95\,\Theta ,
\]
where $\mu$ is the explicit expression evaluated in
\texttt{explicit\_eps0.py}. Since $1-\frac14e^2\le|w_i(t)|\le1$ and
$\langle\varepsilon_i,u_i\rangle=-\frac12e_i^2$, the derivative is at least
$\frac23(1-\lambda_0)^3S+\frac23\sum_ie_i^2-\mu\sum_ie_i$, and integrating,
\[
  \vol V(Y)\ \ge\ 8+\tfrac23(1-\lambda_0)^3S-26.5\,\mu\,S .
\]
...
\[
  \vol(V_c)\ \ge\ \vol(V(Y)\cap K(Y))\ \ge\ 8+S\Bigl(\tfrac23(1-\lambda_0)^3-26.5\,\mu
  -13\,958\cdot6.42^4S^3\Bigr),
\]
and \texttt{explicit\_eps0.py} finds the bracket at least $0.02$ for
$S\le4\cdot10^{-5}$, in exact arithmetic with the expressions for
$\lambda_0$ and $\mu$ at $\Theta=6.42\,S$ (each is increasing in $\Theta$). So
$\vol(V_c)>8$ unless $S=0$. If $S=0$, then $\kappa=0$, so $\varepsilon=0$ by
...
\cref{thm:near-contact} ends at $\sum\delta_i=4\cdot10^{-5}$. With
$\varepsilon_0$ explicit, what closing the rest would take is, at
intermediate distances, a lower bound for the part of the cell outside
$B(\sqrt{3/2})$, which the pair terms of \cref{lem:no-triple} do not see,
strong enough to bridge that factor of about four thousand. The missing
ingredient is the same on both sides of it. The rigidity of
\cref{thm:local-uniqueness} controls the directions only while
$d(W)\le\frac1{48}$, which estimate (a) guarantees only for
$\sum_i\delta_i$ up to about $0.004$; beyond that, twenty-four directions
...
\cref{thm:m24} at moderate slack. \Cref{thm:m24-robust} gives that form only
for slack up to $2\cdot10^{-26}$, and not because of loose constants alone:
the certificate is exact at slack $0$, and its two-point polynomial has
double zeros at $-\frac12$ and $0$, so the distance it controls is the
square root of the error. A form at moderate slack would need a new
computation at the second level of the hierarchy, with a margin, of the
kind carried out in \cite{LLM24}.
...
release \texttt{v1.7.0}, at
\bluelink{https://github.com/creelie/d4-voronoi-cells/releases/tag/v1.7.0}{the
...
\texttt{truncated\_search.py} and \texttt{count\_bound\_sqrt6.py}, the
Lean file \texttt{D4Closure.lean} and the scripts of the figures of that
section, and version \texttt{v1.7.0} adds \texttt{explicit\_eps0.py};
...
$\varepsilon_0=4\cdot10^{-26}$ and the neighbourhood $\sum\delta_i\le4\cdot10^{-5}$
in sixteen seconds. \texttt{count\_bound\_sqrt6.py} proves that at most
...
software, version v1.7.0, Zenodo (2026).
