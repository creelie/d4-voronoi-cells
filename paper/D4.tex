...
twenty-two neighbours lie within $\sqrt6$, whenever the contacts contain
most of a root system, and whenever no neighbour lies at a distance
between $2+\varepsilon_0$ and $\sqrt6$, for an absolute constant
$\varepsilon_0>0$; the last makes the $D_4$ configuration a strict local
minimum of the cell volume among all packings. The case left open is a
centre crowded by neighbours at intermediate distances, for which we give
numerical evidence.
\end{abstract}
...
\item at most $22$ other centres lie within $\sqrt6$ of $c$;
...
\item $c$ has exactly $23$ contacts, and their directions are a deletion
of a copy of the normalised roots or have covering radius at most
$\arccos(\sqrt6/4)=52.24^\circ$;
\item no centre lies at a distance from $c$ strictly between
$2+\varepsilon_0$ and $\sqrt6$, where $\varepsilon_0>0$ is an absolute
constant.
\end{enumerate}
\end{thm}

These are \cref{cor:count,cor:root-subsets,cor:m23-shell,thm:near-contact}.
Case (d) is a local form of the conjecture at the root system, for all
packings: the $D_4$ configuration is a strict local minimum of the cell
volume, whatever the centres beyond $\sqrt6$ do. Its constant
$\varepsilon_0$ comes from a compactness argument and is not explicit, so
what is left of \cref{conj:shell} is a centre with at least twenty-three
other centres within $\sqrt6$, one of them at an intermediate distance,
between $2+\varepsilon_0$ and $\sqrt6$. A numerical search of that case
...
This subsection narrows \cref{conj:shell} in four directions. It removes
every centre with at most twenty-two others within $\sqrt6$, every centre
whose contacts contain most of a root system, every centre with
twenty-three contacts save one family, and every centre around which no
other centre lies at a distance between $2+\varepsilon_0$ and $\sqrt6$,
for an absolute constant $\varepsilon_0>0$; the last is a local form of
the twenty-four-cell conjecture at the root system, for all packings. The
tools are three elementary facts about packings that hold whatever the
distances. Throughout, $c=0$, and the half-space of a centre $z$ is
...
\begin{cor}[Twenty-three contacts]\label{cor:m23-shell}
Let $c$ have exactly $23$ contacts, with directions $W$. If $W$ is a
deletion of a copy of the root system, or if its covering radius on
$S^3$ is at most $\arccos(\sqrt6/4)=52.2388^\circ$, then $\vol(V_c)>8$,
whatever the other centres.
\end{cor}

\begin{proof}
A deletion is \cref{cor:root-subsets} with one root missing. Otherwise,
by \cref{lem:holes}, every centre other than the contacts is at distance
at least $4\cos(52.2388^\circ)=\sqrt6$, so it does not meet
$B(\sqrt{3/2})$ (\cref{lem:shell-new}); hence
$V_c\cap B(\sqrt{3/2})=V_c(W)\cap B(\sqrt{3/2})$, the cell $V_c(W)$ is
bounded (proof of \cref{thm:local-general}), and \cref{thm:certificate}
bounds the volume of that intersection above $8$.
\end{proof}

A contact configuration of $23$ directions whose cell has circumradius
at least $2$ is a deletion (\cref{prop:extendable,thm:m24}). So what
\cref{cor:m23-shell} leaves of the case of $23$ contacts is a
configuration that is not a deletion, whose existence is itself open
(\cref{thm:m23-from-codes}), with a hole of angular radius between
$52.24^\circ$ and $60^\circ$ and another centre in it closer than
$\sqrt6$.

\begin{thm}[The conjecture near the contact regime]\label{thm:near-contact}
There is an absolute constant $\varepsilon_0>0$ with the following
property. If no centre of a unit-ball packing of $\mathbb{R}^4$ lies at a
distance from $c$ strictly between $2+\varepsilon_0$ and $\sqrt6$, then
$\vol(V_c)\ge8$, with equality only if the contacts of $c$ form a copy
of $\sqrt2\,\Rt$ and $V_c$ is the regular $24$-cell they cut out.
\end{thm}

In particular the $D_4$ configuration is a strict local minimum of the
cell volume among all packings: every packing whose centres within
$\sqrt6$ of $c$ are close enough to $\sqrt2\,\Rt'$, for some copy $\Rt'$ of $\Rt$, and
whose other centres are unrestricted, has $\vol(V_c)\ge8$, with equality
only at $\sqrt2\,\Rt'$ itself, and to first order the excess is
$\frac23\sum\delta_i$ (\cref{fig:closure}(c)). The constant
$\varepsilon_0$ comes from a compactness argument and is not explicit.

\begin{proof}
\emph{Step 1: a centre with $\vol(V_c)\le8$ in this regime is close to
the root system.} We show that for every $\eta>0$ there is $\varepsilon>0$
such that, if no centre lies at a distance from $c$ in
$(2+\varepsilon,\sqrt6)$ and $\vol(V_c)\le8$, then the centres within
$\sqrt6$ of $c$ can be matched with the points of $\sqrt2\,\Rt'$, for
some copy $\Rt'$ of $\Rt$, each within $\eta$ of its partner. If not, there are $\eta>0$
and packings $\mathcal P_n$ with a centre at $0$, no centre at a distance
in $(2+1/n,\sqrt6)$, $\vol(V^{(n)})\le8$ for the cell of $0$, and no such
matching. The packing condition bounds the number of centres in every
ball independently of $n$, so after passing to a subsequence the
centres within $B(\rho)$ converge, as finite sets, to those of a packing
$\mathcal P$, for every $\rho$ outside the countable set of distances
at which $\mathcal P$ has a centre (a diagonal argument). In
$\mathcal P$ every centre is at distance $2$ from $0$ or at distance at
least $\sqrt6$. For each such $\rho\ge3$ the set $V^{(n)}\cap B(\rho/2)$ is cut out
by the finitely many centres within $\rho$ and converges in the
Hausdorff metric to $V\cap B(\rho/2)$, $V$ the cell of $0$ in
$\mathcal P$, which contains $B(1)$; so
$\vol(V\cap B(\rho/2))\le\liminf\vol(V^{(n)})\le8$ for all these $\rho$,
and $\vol(V)\le8$. Let $m$ be the number of contacts of
$0$ in $\mathcal P$. If $m\le22$, \cref{cor:count} gives $\vol(V)>8$; if
$m=23$, the centres other than the contacts do not meet
$B(\sqrt{3/2})$, and \cref{thm:certificate} gives
$\vol(V)\ge\vol(V\cap B(\sqrt{3/2}))>8$. So $m=24$, the contacts form a
copy of $\sqrt2\,\Rt'$ (\cref{thm:m24}), and the centres of $\mathcal P_n$ within
$\sqrt6$, which converge to them, can be matched with them within $\eta$
for large $n$, a contradiction.

\emph{Step 2: the root system is a strict local minimum.} Let $Y$ be the
set of centres within $\sqrt6$ of $c$, matched as in Step 1 with the
points of $\sqrt2\,\Rt'$ within a small $\eta$. Rotate so that $\Rt'=\Rt$, write
$u_i$ for the normalised roots and $y_i=d_iw_i$ for the partner of
$2u_i$, with $|w_i|=1$ and $d_i=2+\delta_i$, $0\le\delta_i\le\varepsilon$,
and choose the rotation and the labelling that attain the minimum
$d(W)$ of \cref{thm:local-uniqueness} for $W=\{w_i\}$. Put
$\varepsilon_i=w_i-u_i$, $e_i=|\varepsilon_i|$ and
$\|\varepsilon\|^2=\sum_ie_i^2$, so that $\|\varepsilon\|\to0$ as
$\eta\to0$. Three estimates hold once $\eta$ and $\varepsilon$ are small.

(a) \emph{Rigidity with slack: $\|\varepsilon\|\le6\sum_i\delta_i$.} The
proof of \cref{thm:local-uniqueness} goes through with the contact
condition $\langle w_i,w_j\rangle\le\frac12$ on the $96$ pairs of $T$
replaced by the packing condition $|y_i-y_j|\ge2$, which reads
$\langle w_i,w_j\rangle\le\frac12+\kappa_{ij}$ with
$\kappa_{ij}=(d_i^2+d_j^2-d_id_j-4)/(2d_id_j)
\le\frac14(\delta_i+\delta_j)+\frac18(\delta_i+\delta_j)^2$. The positive
parts of $\Lambda\tau$ then sum to at most
$6\|\varepsilon\|^2+\kappa$, $\kappa=\sum_T\kappa_{ij}$; the constant
stress still makes the entries of $\Lambda\tau$ sum to $0$, so
$\|\tau\|_2\le\|\Lambda\tau\|_1\le12\|\varepsilon\|^2+2\kappa$ by
\eqref{eq:sigma-one}, and with $\sum_i\rho_i^2\le\frac14\|\varepsilon\|^4$,
$\|\varepsilon\|\le\frac14\|\varepsilon\|+2\kappa+\frac12\|\varepsilon\|^2$.
For $\|\varepsilon\|\le1/48$ this gives $\|\varepsilon\|\le\frac{192}{71}\kappa$,
and every index lies in eight pairs of $T$, so
$\kappa\le2(1+\varepsilon)\sum_i\delta_i$.

(b) \emph{The volume to first order: $\vol(V(Y))\ge8+\frac23\sum_i\delta_i
-C\bigl(\|\varepsilon\|+\sum_i\delta_i\bigr)^2$.} Move the data linearly,
$w_i(t)=u_i+t\varepsilon_i$ and $h_i(t)=1+t\delta_i/2$ for $0\le t\le1$,
and let $P(t)=\{x:\langle x,w_i(t)\rangle\le h_i(t)\}$, so that $P(0)=Q$
and $P(1)=V(Y)$. Each facet of $P(t)$ moves with normal velocity
$(\dot h_i-\langle x,\dot w_i\rangle)/|w_i|$, so
\[
  \frac{d}{dt}\vol P(t)=\sum_i\frac{1}{|w_i(t)|}
  \Bigl(\frac{\delta_i}{2}A_i(t)-\langle\varepsilon_i,M_i(t)\rangle\Bigr),
\]
$A_i$ the $3$-volume of the $i$-th facet and $M_i=\int x\,dA$ over it. At
$t=0$ every facet is a regular octahedron of volume $\frac43$ centred at
$u_i$, so the right side is
$\frac23\sum_i\delta_i-\frac43\sum_i\langle\varepsilon_i,u_i\rangle
=\frac23\sum_i\delta_i+\frac23\sum_ie_i^2$, by \eqref{eq:radial-part}.
Each facet of $P(t)$ is the solution set, in its hyperplane, of a system
of linear inequalities whose coefficients depend smoothly on $t$, and at
$t=0$ its centre satisfies every inequality with a margin of at least
$\frac12$; shrinking a solution set towards such an interior point by the
factor $1-\lambda$ keeps it feasible after a perturbation of size
$O(\lambda)$, so the facet moves by $O(\|\varepsilon\|+\max_i\delta_i)$
in the Hausdorff metric, uniformly for $t\le1$, and so do $A_i$, $M_i$
and $|w_i|$. Integrating the derivative gives (b).

(c) \emph{The hull costs only a fourth-order amount:
$\vol(V(Y)\setminus K(Y))\le C\bigl(\|\varepsilon\|+\max_i\delta_i\bigr)^4$.}
For $x\in V(Y)$, $\langle x,u_i\rangle\le h_i+|x|e_i$, so
$V(Y)\subseteq(1+\theta)Q$ with $\theta=O(\|\varepsilon\|+\max_i\delta_i)$.
The points $4w_i/d_i$ spanning $K(Y)$ lie within $\delta_i+2e_i$ of
$2u_i$, and the convex hull of the $2u_i$ has inradius $\sqrt2$, so
$K(Y)\supseteq(1-\theta')\conv\{2u_i\}$ with $\theta'$ of the same order.
$Q$ lies in $\conv\{2u_i\}$ and meets its boundary only at its $24$
vertices, each of which is the unique point of $Q$ on the supporting
hyperplane there, because $v=r+r'$ is a positive combination of the six
roots at $v$ and so lies in the interior of the normal cone of $Q$ at
$v$. Hence $(1+\theta)Q\setminus(1-\theta')\conv\{2u_i\}$ consists of $24$
caps of height $O(\theta+\theta')$ cut from the vertex cones of $Q$,
whose volume is $O((\theta+\theta')^4)$.

By \cref{prop:inversion-hull}, (b), (c) and (a),
\[
  \vol(V_c)\ \ge\ \vol(V(Y)\cap K(Y))\ \ge\ 8+\tfrac23\sum_i\delta_i
  -C'\Bigl(\sum_i\delta_i\Bigr)^2 ,
\]
which is at least $8$ once $\sum_i\delta_i\le2/(3C')$, with equality only
if every $\delta_i=0$. Then $\kappa=0$, so $\varepsilon=0$ by (a), $Y=\sqrt2\,\Rt$,
and $V_c\supseteq V(Y)\cap K(Y)=Q$ with $\vol(V_c)=8$, so $V_c=Q$.
Choosing $\eta$ and then $\varepsilon$ in Step 1 small enough for (a), (b)
and (c) gives $\varepsilon_0$.
\end{proof}

\emph{What is left, and the evidence.} Together with
\cref{prop:distance-criterion}, the corollaries above and
\cref{thm:near-contact} confine \cref{conj:shell} to a centre $c$ with all
of the following: at least $23$ other centres within $\sqrt6$; at least
one of them at a distance between $2+\varepsilon_0$ and $\sqrt6$;
$\Phi\le8$ and $\Psi\le8$; contacts that do not contain one orthogonal
pair of roots at each vertex of a $24$-cell; and, if there are $23$
contacts, a configuration that is not a deletion, with a hole of angular
radius above $52.24^\circ$. Near the root system the inversion hull is
...
$-\frac12$, the value on the deleted root's axis. Closing what is left
would take an explicit $\varepsilon_0$, which needs a quantitative form
of \cref{thm:m24}, and a proof for the intermediate distances, for which
the pair terms of \cref{lem:no-triple} are exact and a three-point bound
on the shell, as in \cref{thm:certificate} for the sphere, is the natural
tool.
...
waits. By \cref{sec:closure} it is a centre with at least twenty-three
others within $\sqrt6$, one of them at a distance between
$2+\varepsilon_0$ and $\sqrt6$; closer to contact the bound holds
(\cref{thm:near-contact}). Closing it would take an explicit
$\varepsilon_0$, which needs a quantitative form of \cref{thm:m24}, and,
for the intermediate distances, what the proof of \cref{thm:m23} does for
equal caps, an exact account of the pairs, which \cref{lem:no-triple}
provides for every packing, with the three-point method carried from the
sphere to a shell. The
...
of the squared distances and the minimality of the centroid, over every
commutative ring, by the ring normaliser of Lean's \texttt{grind}, so
that they hold verbatim over $\mathbb{R}$; and it proves by kernel
computation (\texttt{decide +kernel}) the finite content of
...
release \texttt{v1.5.0}, at
\bluelink{https://github.com/creelie/d4-voronoi-cells/releases/tag/v1.5.0}{the
tag of that name}; the previous release, \texttt{v1.4.0}, has the version DOI
\bluelink{https://doi.org/10.5281/zenodo.22940045}{10.5281/zenodo.22940045}.
The Zenodo record that collects every version, and
resolves to the most recent of them, has the concept DOI
...
alone. Version \texttt{v1.4.0} adds an independent re-verification of
every computation of the paper, in the directory
\texttt{independent\_verification} with its report and the logs of every
run, the two programs of \cref{sec:noncontact}
(\texttt{shell\_reduction.py} and \texttt{shell\_neighbour\_search.py}),
and the corrections to the supplement that the re-verification found.
The code of the re-verification, those two programs and the corrections
were developed with the assistance of Claude, an AI model made by
Anthropic. Version \texttt{v1.5.0} adds the two programs of
\cref{sec:closure}, \texttt{closure\_lemmas.py} and
\texttt{near\_contact\_probe.py}, developed in the same way.
...
\cref{lem:holes,prop:inversion-hull,lem:no-triple}, proved over every
commutative ring by the ring normaliser of \texttt{grind}, and the
orthogonal root pairs and deleted sets of \cref{cor:root-subsets}, by
\texttt{decide +kernel}; it checks in about a minute and a half, and
...
\Cref{sec:closure} rests on two more. \texttt{closure\_lemmas.py} checks
...
$\Psi$ beside those of $\Phi$, and the angle $\arccos(\sqrt6/4)$ of
\cref{cor:m23-shell}. \texttt{near\_contact\_probe.py} is floating point
...
the package: the Lean verifications (seven at the time;
\texttt{D4Closure.lean} was added in v1.5.0), the certificates for $23$ and
...
software, version v1.5.0, Zenodo (2026).
\bluelink{https://doi.org/10.5281/zenodo.22766562}{doi:10.5281/zenodo.22766562}
(concept DOI: the record of all versions, which resolves to the latest);
previous version v1.4.0:
\bluelink{https://doi.org/10.5281/zenodo.22940045}{doi:10.5281/zenodo.22940045}.
Source repository:
\bluelink{https://github.com/creelie/d4-voronoi-cells}{github.com/creelie/d4-voronoi-cells}
