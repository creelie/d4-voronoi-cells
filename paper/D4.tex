...
neighbour lies at a distance between $2+\varepsilon_0$ and $\sqrt6$, for an
absolute constant $\varepsilon_0>0$, which makes the $D_4$ configuration a
strict local minimum of the cell volume among all packings. The case left
...
what is left of \cref{conj:shell} is a centre with at least twenty-four
...
$\varepsilon_0>0$, which is a local form of the twenty-four-cell
conjecture at the root system, for all packings. The tools are three
...
$0$ in $\mathcal P$. If $m\le23$, the only centres of $\mathcal P$
within $\sqrt6$ of $0$ are its $m$ contacts, and
\cref{thm:m23-labelled} gives $\vol(V)>8$. So $m=24$, the contacts form a
...
$\sqrt6$, from twelve starts each. For $M=24$ every run ends at the root
system, at $7.906940\ldots$; for $M=25$, $26$ and $27$ every local minimum
it finds lies above $8.26$. If that is the whole truth, the pair terms fail
...
count up to the largest that fits within $\sqrt6$. Closing what is left
would take an explicit $\varepsilon_0$, which needs a quantitative form
of \cref{thm:m24}, and, at intermediate distances, a lower bound for the
part of the cell outside $B(\sqrt{3/2})$, which the pair terms of
\cref{lem:no-triple} do not see.
...
at intermediate distances, a bound for the part of the cell outside
$B(\sqrt{3/2})$, since inside that ball the root system itself falls
short of $8$. The
...
release \texttt{v1.6.0}, at
\bluelink{https://github.com/creelie/d4-voronoi-cells/releases/tag/v1.6.0}{the
tag of that name}; the release \texttt{v1.4.0} has the version DOI
...
Anthropic. Versions \texttt{v1.5.0} and \texttt{v1.6.0} add the
programs of \cref{sec:closure}, \texttt{closure\_lemmas.py},
\texttt{near\_contact\_probe.py} and
\texttt{labelled\_certificate\_check.py}, the Lean file
\texttt{D4Closure.lean} and the scripts of the figures of that section;
these too were developed with the assistance of Claude, and their
...
$\varepsilon_0$ & \parbox[t]{0.76\textwidth}{\raggedright the absolute constant of \cref{thm:near-contact}}\\
...
\Cref{sec:closure} rests on three more. \texttt{closure\_lemmas.py} checks
...
about six minutes. \texttt{near\_contact\_probe.py} is floating point
...
software, version v1.6.0, Zenodo (2026).
