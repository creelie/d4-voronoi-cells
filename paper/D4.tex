...
inner products at most $\tfrac12+0.0065$, so at most twenty-four neighbours lie
within $2.0131$ of any centre. The case left
...
at most twenty-four lie within $2.0131$: the kissing number is stable under
a slack of $0.0065$ in the inner products (\cref{thm:kissing-stable}), which
...
$\sum_i\delta_i\le6\cdot10^{-5}$, and their directions $W$ satisfy
...
(a) \emph{Rigidity with slack: $\|\varepsilon\|\le4.24\,(1+\delta)S$.} The
...
$\|\tau\|_2\le\frac{\sqrt{11}}4\|\Lambda\tau\|_1\le3\sqrt{11}\|\varepsilon\|^2+\frac{\sqrt{11}}2\kappa$ by
\eqref{eq:sigma-one} and \eqref{eq:image-linf}, and with $\sum_i\rho_i^2\le\frac14\|\varepsilon\|^4$,
$\|\varepsilon\|\le(3\sqrt{11}+\frac12)\|\varepsilon\|^2+\frac{\sqrt{11}}2\kappa$.
For $\|\varepsilon\|\le1/48$ this gives $\|\varepsilon\|\le c_a\kappa$ with
$c_a=\frac{\sqrt{11}}2\big/\bigl(1-\frac{1}{48}(3\sqrt{11}+\frac12)\bigr)<2.1199$,
and every index lies in eight pairs of $T$, so
$\kappa\le2(1+\delta)S$. Consequently
$\sum_ie_i\le\sqrt{24}\,\|\varepsilon\|\le20.8\,S$ and
$\Theta\le\|\varepsilon\|+S\le5.24\,S\le3.2\cdot10^{-4}$.
...
\[
  \vol V(Y)\ \ge\ 8+\tfrac23(1-\lambda_0)^3S-20.8\,\mu\,S .
\]
...
\[
  \vol(V_c)\ \ge\ \vol(V(Y)\cap K(Y))\ \ge\ 8+S\Bigl(\tfrac23(1-\lambda_0)^3-20.8\,\mu
  -13\,958\cdot5.24^4S^3\Bigr),
\]
and \texttt{explicit\_eps0.py} finds the bracket at least $0.02$ for
$S\le6\cdot10^{-5}$, in exact arithmetic with the expressions for
$\lambda_0$ and $\mu$ at $\Theta=5.24\,S$ (each is increasing in $\Theta$). So
...
\cref{thm:near-contact} ends at $\sum\delta_i=6\cdot10^{-5}$. Letting the
...
two and a half thousand. The missing ingredient is the same on both sides of
it. The rigidity of \cref{thm:local-uniqueness} controls the directions only
while $d(W)\le\frac1{48}$, which estimate (a) guarantees only for
$\sum_i\delta_i$ up to about $0.005$; beyond that, twenty-four directions
...
\cref{thm:kissing-stable} excludes a twenty-fifth point up to slack $0.0065$,
so at most $24$ other centres lie within $2.0131$ of any centre. Its shape
...
Let $s_0=0.0065$. Every set of points of $S^3$ whose pairwise inner
...
centres lie within distance $2/\sqrt{1-2s_0}=2.0131\ldots$ of any centre.
\end{thm}
...
The certificate has degree $d=8$. It was found by solving the
corresponding programme in floating point, with (i) and (ii) imposed at
sampled points and refined in rounds, $f(1)+F(1,1,1)$ held at $23.975$ and
...
$LDL^{\mathsf T}$ factorisation; the value $f(1)+F(1,1,1)=23.974999950\ldots$,
exactly; (i) with $e_1=10^{-6}$, by branch and bound on the exact univariate
polynomial; and (ii) with $e_2=10^{-5}$ on the ordered admissible domain
...
about its centre. Both domains are run up to the least double at or above
$t$. Then $24(1-e_1)-552e_2=23.994456>23.975$.
...
by an explicit amount. A degree-$8$ certificate at $s_0=0.005$ passes the
same check, and \cref{fig:positive-slack}(a) shows why the method stops soon after:
...
optimistic), with the two certified values of \cref{thm:kissing-stable} and
its companion at $s=0.005$. It crosses $25$ near $s=0.009$; a $25$-point
code exists from $s=0.0374$ on. (b) How far the proof of
...
\texttt{robust\_ceiling.py} and \texttt{code25\_search.py}), the
...
$\varepsilon_0=4\cdot10^{-26}$ and the neighbourhood $\sum\delta_i\le6\cdot10^{-5}$
in about fifteen seconds. \texttt{certify\_cardinality.py} is the proof of
...
degree $8$ it takes about eighty seconds. \texttt{cardinality\_sdp.py}, which
