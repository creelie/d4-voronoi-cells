...
paper closes, and where, can be said exactly. For contact
configurations, the directions to the neighbours at distance exactly
$2$, the bound $\vol\ge8$ with equality only at the root system is proved
in full: simultaneous deviation in several directions, which the
preprint assumed away, is settled by the contact count, at most $22$
contacts by \cref{thm:local-fewcontacts}, exactly $23$ by
\cref{thm:m23} and exactly $24$ by \cref{thm:m24}, and that is
\cref{cor:conj-resolved}. The single-deviation bound, with no
restriction on the deviation, is \cref{thm:local-uncond}. For the
Voronoi cell of a packing the bound is proved whenever the centres
within $2\sqrt2$ of the given one all touch it, or their distances pass
the covering criterion of \cref{prop:distance-criterion}; that is
\cref{thm:local-general}, and \cref{thm:main-general} is the density
bound it gives. The case left open is a centre crowded by at least
twenty-three neighbours within $2\sqrt2$ some of which do not touch it
(\cref{conj:shell}), and the density bound $\Delta_4=\pi^2/16$ for all
packings follows once that case is settled. An earlier version of this
paper, with version v1.3.0 of its supplement, asserted the general bound
without that hypothesis, through a reduction to contact configurations
(\cref{lem:shell-new,lem:radial-new} in their earlier form) that fails
when a neighbour does not touch the centre; \cref{sec:noncontact} gives
the reduction that holds and the examples that show the earlier one
fails. The present version goes with the new release v1.4.0 of the
supplement, archived on Zenodo as
...
\cite{D4Zenodo}. One more point of provenance needs saying. The classification of $24$-point
...
not look at their directions. The case left open is a centre crowded by
near-contacts, for which we give numerical evidence.
\end{abstract}
...
$2.0705$). A numerical search of that case finds nothing below $8$
(\cref{sec:noncontact}); that is evidence, not proof.
...
What remains of the twenty-four-cell conjecture is \cref{conj:shell}, and
the density bound $\Delta_4=\pi^2/16$ for all packings waits on it.
...
exists, with no appeal to the conditioning of any solver. That rational
measure is not in the supplement, so a reader cannot recheck it from
there; the supplement carries the floating-point measure of
\texttt{m24\_primal\_sdp.py}, of value $5.9424$ on $2170$ triples as
deposited and $5.9851$ on $522$ triples when rerun with a later solver,
both below $\tau$ by more than $0.18$, which is evidence of the same
conclusion rather than a proof of it.
...
waits. Closing it would take, for caps of unequal radii, what the proof
of \cref{thm:m23} does for equal ones, an exact account of the pairs,
with the three-point method carried from the sphere to a shell. The
...
the new release \texttt{v1.4.0}, of 24 September 2026, at
\bluelink{https://github.com/creelie/d4-voronoi-cells/releases/tag/v1.4.0}{the
tag of that name}, archived on Zenodo with the version DOI
\bluelink{https://doi.org/10.5281/zenodo.22940045}{10.5281/zenodo.22940045}.
The Zenodo record that collects every version, and
resolves to the most recent of them, now \texttt{v1.4.0}, has the concept DOI
\bluelink{https://doi.org/10.5281/zenodo.22766562}{10.5281/zenodo.22766562}
...
the package: the seven Lean verifications, the certificates for $23$ and
...
software, version v1.4.0 (released 24 September 2026), Zenodo (2026).
