...
(\texttt{count\_bound\_sqrt6.py}). With $\varepsilon_0$ explicit, what
closing the rest would take is, at intermediate distances, a lower bound
for the part of the cell outside $B(\sqrt{3/2})$, which the pair terms of
\cref{lem:no-triple} do not see, strong enough to meet the explicit
neighbourhood of \cref{thm:near-contact}.
...
these sums from the deposited data in ball arithmetic, bounding the
largest eigenvalue of each positive definite block by its trace or its
largest absolute row sum and each entry of $m$ by the $\ell^1$ norm of its
coefficients in the tensor Chebyshev basis, which is at least its
supremum on $[-1,1]^6$; it finds $B_3\le8.75\cdot10^{7}$ and
...
Gram matrix of the normalised roots in some order, and for an orthogonal
map $R$,
\[
...
$-1$, hence nonnegative, and $2g$ is positive semidefinite. It is
...
$[-1,1]^6$; and the lemma asks for inner products within $3\cdot10^{-4}$
of the four values where a tenth of that would be plenty, because it
deduces integrality from a single determinant. What matters for
