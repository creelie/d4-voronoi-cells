...
`D4Meet.lean` covers the finite half of Propositions 7.43 and 7.44: how much
...
`D4Certificate.lean` covers the exact half of the proof of Theorem 7.61,
...
inequality of Lemma 7.60 on the admissible triples is verified in interval
arithmetic by `multi_cap/certificate_check.py` and is not formalised.

`certificate/` covers the branch and bound in the proof of Theorem 7.61 a
...
| `deletion_shortfall` | with the integration stopped at r_23 the deletion configuration is 3 pairs short of the 91 the estimate would need (Remark 7.33) |
| `deletion_surplus` | carried to r_* the estimate needs 65 (Proposition 7.32), and the deletion has 23 more |
| `tight_count24` | the full root system has 96 tight pairs, the edges of the 24-cell |
| `equilibrium24` | the constant stress, weight 1 on every tight pair and -4 on the diagonal, is in equilibrium: the eight neighbours of every root sum to four times it (the corollary after Proposition 7.39) |
...
| `two_roots_leave_a_couple` | hence so does any pair of removed roots, which is the finite half of Proposition 7.43 |
...
half of Propositions 7.43 and 7.44 are also not formalised; they are short
