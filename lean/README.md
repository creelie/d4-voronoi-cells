...
`D4Stress.lean` covers Proposition VII.45 and the corollary that follows it:
...
`D4Meet.lean` covers the finite half of Propositions VII.52 and VII.53: how much
...
`D4Certificate.lean` covers the exact half of the proof of Theorem VII.70,
...
inequality of Lemma VII.69 on the admissible triples is verified in interval
arithmetic by `multi_cap/certificate_check.py` and is not formalised.

`certificate/` covers the branch and bound in the proof of Theorem VII.70 a
...
certificate of de Laat, Leijenhorst and de Muinck Keizer (Theorem VII.25 and
Remark VII.27): the two-point polynomial p_2 of that certificate, computed
...
`cell600/` covers Proposition VII.60: every set of twenty-three vertices of
...
the four files there is no `lakefile`: install elan and run
...
| `equilibrium` | the equilibrium relation (VII.28) holds at each of the 23 directions |
| `pair_ceiling` | a graph on 23 vertices of maximum degree 10 has at most 115 edges |
| `deletion_shortfall` | with the integration stopped at r_23 the deletion configuration is 3 pairs short of the 91 the estimate would need (Remark VII.39) |
| `deletion_surplus` | carried to r_* the estimate needs 65 (Proposition VII.38), and the deletion has 23 more |
| `tight_count24` | the full root system has 96 tight pairs, the edges of the 24-cell |
| `equilibrium24` | the constant stress, weight 1 on every tight pair and -4 on the diagonal, is in equilibrium: the eight neighbours of every root sum to four times it (the corollary after Proposition VII.45) |
...
| `two_roots_leave_a_couple` | hence so does any pair of removed roots, which is the finite half of Proposition VII.52 |
...
half of Propositions VII.52 and VII.53 are also not formalised; they are short
...
Proposition VII.60 about those 120 vectors is checked. `roots_inner` is
