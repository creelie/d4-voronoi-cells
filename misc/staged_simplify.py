...
for g in range(5):
    ...
    print(f"group {g} (n={n} terms): cancel() {status} in {dt:.1f}s, "
          f"numeric@t0val={val:.4f}")

with open(_os.path.join(_DATA_DIR, 'group_totals.pkl'), 'wb') as f:
    pickle.dump(group_totals, f)
print("saved group_totals.pkl to %s" % _DATA_DIR)
